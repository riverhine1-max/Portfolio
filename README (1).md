...
**Current site:** serve the repository (for example `python3 -m http.server 8123`) and run `node tests/studio.cjs`. It loads every page at desktop and mobile widths and checks for script errors, missing files, horizontal overflow, broken local links, and that the data-driven reel and Work grid render.

The other scripts below were written for the previous design and target its old markup; they are kept for reference.
...
The motion check covers five viewport widths, featured-project switching, keyboard arrows, touch input, the scroll/click walkthrough, motion controls, reduced-motion video loading, console errors, and no-JavaScript fallbacks.
