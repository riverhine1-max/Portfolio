...
Everything on the Work page — grid, index, filters, Then → Now timeline, project count, and the homepage reel — is rendered from **`assets/projects.js`**.

1. Add a cover (~1600px wide) and a thumbnail (~800px) to `Images/`.
2. Copy the `TEMPLATE` block at the top of `assets/projects.js`, paste it into `window.PROJECTS`, and fill it in.
...
- `assets/studio.css` — one stylesheet for every page (tokens, type, components, responsive rules, reduced motion, print).
- `assets/studio.js` — motion engine: loader, page-transition curtain, Lenis smooth scroll, custom cursor, magnetic buttons, split-text reveals, scroll-scrubbed reel and chapters, marquee velocity, counters, video players, systems explorer, and the video carousel.
- `assets/work.js` — Work page rendering and the project viewer dialog.
- Fonts are self-hosted in `assets/` (Archivo, Instrument Serif, JetBrains Mono — SIL Open Font License). `assets/lenis.min.js` is Lenis 1.3 (MIT).
