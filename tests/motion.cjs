...
for(const width of [1440,1024,768,390,360]){
 ...
 await p.locator('.world-motion').click();
...
await p.getByRole('button',{name:'Play motion',exact:true}).click();await p.waitForFunction(()=>document.querySelector('#world-film').currentTime>0);await p.getByRole('button',{name:'Pause motion',exact:true}).click();ok(await p.locator('#world-film').evaluate(v=>v.paused),'pause control');
