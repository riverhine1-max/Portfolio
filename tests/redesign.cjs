...
(async () => {
  ...
  const film = page.locator('#world-film');
  await page.getByRole('button',{name:'Play motion'}).click();
  await page.waitForFunction(()=>{const v=document.querySelector('#world-film');return v.currentTime>0&&!v.paused});
  assert(await film.evaluate(v=>v.videoWidth>0),'Higgsfield film playback');
  await film.evaluate(v=>{v.currentTime=2.5});
  await page.waitForFunction(()=>!document.querySelector('#world-film').seeking);
  await page.locator('.motion-hero').screenshot({path:'test-results/brand-film.png'});
