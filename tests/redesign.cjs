...
(async () => {
  ...
  assert(await page.locator('#brand-film').getAttribute('src')===null,'ambient video should not load automatically');
  assert(await page.locator('#brand-film').evaluate(v=>v.paused),'reduced-motion film paused');
  const film = page.locator('#brand-film');
  await page.getByRole('button',{name:'Play ambient film'}).click();
  await page.waitForFunction(()=>{const v=document.querySelector('#brand-film');return v.currentTime>0&&!v.paused});
  assert(await film.evaluate(v=>v.videoWidth>0),'Higgsfield film playback');
  await film.evaluate(v=>{v.currentTime=2.5});
  await page.waitForFunction(()=>!document.querySelector('#brand-film').seeking);
  await page.locator('.brand-interlude').screenshot({path:'test-results/brand-film.png'});
  await page.getByRole('button',{name:'Pause ambient film'}).click();
  assert(await film.evaluate(v=>v.paused),'Higgsfield film pause');
